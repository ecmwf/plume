...
// the buffer, so the scope only borrows the tracker (to report an aborted write) and the model buffer pointer.

WriteScope::WriteScope(coupling::WriteBackTracker& tracker, std::string name, atlas::Field& field) :
    tracker_(&tracker), name_(std::move(name)), field_(&field) {}
...
WriteScope::~WriteScope() {
    if (tracker_ != nullptr && !committed_) {
        valid_ = false;  // poison any outstanding FieldWriter before reporting
        try {
            tracker_->reportError(name_, "write scope destroyed without commit()");
        }
        catch (...) {
            // A destructor must never throw; the tracker error is best-effort here.
        }
    }
}
...
void WriteScope::commit() {
    ...
    // separate buffer to flush. Committing simply poisons the handle and leaves the tracker slot STAGED for
