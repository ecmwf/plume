...
WriteScope::WriteScope(coupling::WriteBackTracker& tracker, std::string name,
                       ParameterValueTyped<atlas::Field>& paramValue) :
    tracker_(&tracker), name_(std::move(name)), field_(&paramValue.getSettableField()), paramValue_(&paramValue) {}
...
void WriteScope::commit() {
    ...
    valid_     = false;
    committed_ = true;
    field_->set_dirty(); // Mark the field as dirty for potential future halo exchange
