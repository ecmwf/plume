 ...
 * of ModelData — reading parameters (getParam), writing model values back through the authorised tracker (writeParam),
 * and querying availability/updated state — while making the model-only mutation entry points (updateParam,
 * provideParam, createParam, ...) and the Manager-facing lifecycle (filter, tracker attach/enroll, acknowledge/pending,
 * ...) inaccessible.
 *
 ...
 *       self-contained and it carries the write-back tracker pointer and consumer tag set by filter().
 *
...
public:
    ...
    // handle and bypass the write-back tracker. The call syntax is unchanged: getParam<atlas::Field>(name) still
