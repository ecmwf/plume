...
namespace coupling {
class WriteBackTracker;  // forward declaration — a WriteScope reports an aborted write back to the tracker.
}
 ...
 * cannot copy out a mutable handle and bypass the write-back tracker. If a plugin does need a mutable field (e.g. for
...
public:
    ...
    // it in place, bypassing the write-back tracker. clone() is the sanctioned way to get a mutable field from a View.
 ...
 * write with the authorisation tracker; an unauthorised plugin never obtains a FieldWriter at all.
 *
 ...
 * writeParam stages the write with the authorisation tracker (throwing if the plugin is not authorised or the
 ...
 * destructor poisons the handle and reports the failure to the tracker so the missing write is not silently lost.
 *
 * @code
...
public:
    ...
    /**
     * @brief Finalises the scope: poisons any outstanding FieldWriter, dirties field, and marks the parameter updated.
     *
     * It leaves the tracker slot staged for the model to flush. A repeat call is a no-op.
     */
    void commit();

private:
    // Only ModelData::writeParam(name) constructs a WriteScope, and only after it has staged the write with the
    // tracker — so a scope can never exist for an unauthorised or unstaged write. It borrows the tracker and a non-owning
    // pointer to the parameter, resolving the model buffer from it.
    friend class ModelData;
    WriteScope(coupling::WriteBackTracker& tracker, std::string name, ParameterValueTyped<atlas::Field>& paramValue);

    coupling::WriteBackTracker* tracker_;  // null after a move; guards the destructor abort path
    std::string name_;
