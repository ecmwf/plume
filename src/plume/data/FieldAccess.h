 ...
 * reports the failure to the tracker so the missing write is not silently lost.
 *
 * @code
...
public:
    ...
    /// Finalises the scope: poison any outstanding FieldWriter and leave the tracker slot staged for the model flush.
    void commit();

private:
    // Only ModelData::writeParam(name) constructs a WriteScope, and only after it has staged the write with the
    // tracker — so a scope can never exist for an unauthorised or unstaged write. The scope itself needs no access
    // to ModelData's internals: it borrows the already-resolved buffer and the tracker (for the abort report).
    friend class ModelData;
    WriteScope(coupling::WriteBackTracker& tracker, std::string name, atlas::Field& field);
