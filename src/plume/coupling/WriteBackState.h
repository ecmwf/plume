 ...
 * make sharing convenient. In practice, WriteBackTracker obtains states exclusively through these accessors and never
...
const WriteBackState* written();
const WriteBackState* pending();
const WriteBackState* acknowledged();
 ...
 *   READY        -> WRITTEN      via onWrite()
 *   READY        -> IDLE         via onSubmit()       — no plugin chooses to write this cycle (not an error)
 *   READY        -> ERROR        via onError()        — tracker detected a failure
 *   WRITTEN      -> WRITTEN      via onWrite()        — multi-writer: subsequent plugins accumulate writes
 *   WRITTEN      -> PENDING      via onSubmit()
 *   WRITTEN      -> ERROR        via onError()
 *   PENDING      -> ACKNOWLEDGED via onAcknowledge()
 *   ACKNOWLEDGED -> IDLE         via onReset()
 *   ERROR        -> IDLE         via onClearError()
 *   ERROR        -> ERROR        via any other trigger (stays poisoned; swallows trigger silently to preserve
 *                                the original error, e.g. submit() called after a failed write() won't mask it)
 *
 * Caller responsibilities:
 *   - Single-writer policy: tracker enforces this before calling onWrite() on a WRITTEN slot.
 *   - Error detection: tracker calls onError() instead of onSubmit() when it detects a failure.
 */
class WriteBackState {
public:
    ...
    virtual const WriteBackState* onWrite() const { invalidTransition("onWrite"); }
    virtual const WriteBackState* onSubmit() const { invalidTransition("onSubmit"); }
    ...
    virtual bool isWritten() const { return false; }
    virtual bool isPending() const { return false; }
    virtual bool isAcknowledged() const { return false; }
...
/**
 * Tracker has opened this slot; an authorised plugin may call write(). Reaching onSubmit() without any write silently
...
public:
    ...
    const WriteBackState* onWrite() const override { return written(); }
    const WriteBackState* onSubmit() const override { return idle(); }  // silent reset: not an error
 ...
 *
 * onWrite() returns WRITTEN again to support multi-writer policy (subsequent plugins accumulate writes sequentially).
 * Single-writer enforcement is the tracker's responsibility.
 * onSubmit() is state-only - no data movement (write already applied).
 */
class Written : public WriteBackState {
public:
    const char* name() const override { return "WRITTEN"; }
    bool isWritten() const override { return true; }

    const WriteBackState* onWrite() const override { return written(); }  // multi-writer: stays WRITTEN
    const WriteBackState* onSubmit() const override { return pending(); }
    const WriteBackState* onError() const override { return writebackError(); }
};
...
class Pending : public WriteBackState {
public:
    const char* name() const override { return "PENDING"; }
    bool isPending() const override { return true; }

    const WriteBackState* onAcknowledge() const override { return acknowledged(); }
};

// =============================================================================

/// Model has acknowledged the write. Slot may be reset to IDLE for the next cycle.
class Acknowledged : public WriteBackState {
public:
    const char* name() const override { return "ACKNOWLEDGED"; }
    bool isAcknowledged() const override { return true; }
...
public:
    ...
    const WriteBackState* onWrite() const override { invalidTransition("onWrite"); }
    const WriteBackState* onSubmit() const override { return writebackError(); }
...
inline const WriteBackState* written() {
    static Written s;
    return &s;
}
inline const WriteBackState* pending() {
    static Pending s;
    return &s;
}
inline const WriteBackState* acknowledged() {
    static Acknowledged s;
    return &s;
}
