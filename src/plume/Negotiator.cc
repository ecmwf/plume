...
void Negotiator::logSummary() const {
    ...
    eckit::Log::info() << "Plume negotiation summary — accepted params claimants:" << std::endl;
    std::map<std::string, std::vector<std::pair<std::string, std::string>>> claimantsByParam;
    auto mergeClaimants = [&claimantsByParam](const std::map<std::string, std::vector<std::string>>& claimants,
                                              const std::string& mode) {
        for (const auto& [pname, plugins] : claimants) {
            for (const auto& pluginName : plugins) {
                claimantsByParam[pname].emplace_back(pluginName, mode);
            }
        }
    };
    mergeClaimants(writableClaimants_, "write");
    mergeClaimants(readonlyClaimants_, "read");
    mergeClaimants(derivedClaimants_, "derived");

    for (const auto& [pname, claimants] : claimantsByParam) {
        eckit::Log::info() << "  " << pname << ":" << std::endl;
        for (const auto& [pluginName, mode] : claimants) {
            eckit::Log::info() << "       - " << pluginName << "  (" << mode << ")" << std::endl;
        }
    }
}
