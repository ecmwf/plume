...
! differently, but there is the same guarantee (a plugin handle has no mutator to call).
...
function plume_data_get_shared_atlas_field( handle, name, field ) result (err)
  ...
  ! here — a Fortran plugin can still mutate the field in place and bypass the write-back tracker. Extending the
