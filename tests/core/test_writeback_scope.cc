...
/// granting the listed consumers write access to "F", and a tracker enrolled (and, by default, attached) under the
/// given policy. `tracker.open()` and view filtering stay in each CASE so their ordering remains explicit.
struct WritebackHarness {
    ...
    coupling::WriteBackTracker tracker;
    ...
    void attachTracker() { data.attachWritebackTracker(&tracker); }

    plume::data::ModelDataView view(const std::string& consumer) {
        return data.filter(std::set<std::string>{"F"}, consumer);
    }

    /// Successful teardown: flush the tracker, acknowledge the write-back, detach.
    void flushAndDetach() {
        tracker.submit();
        data.acknowledgeWriteback("F");
        data.detachWritebackTracker();
    }

    /// Failure teardown: force the tracker back to a clean state, detach.
    void resetAndDetach() {
        ManagerTestAccess::forceTrackerReset(tracker);
        data.detachWritebackTracker();
    }
};
...
CASE("test writeback scope - manual WriteScope writes in place and preserves the FieldImpl") {
...

    {
        ...
        EXPECT_NOT(h.modelField.dirty());
        for (int i = 0; i < 4; ++i) {
            v(i) *= 10;  // 10, 20, 30, 40 — in place
        }
        scope.commit();
        EXPECT(h.modelField.dirty());
    }
...
CASE("test writeback scope - callable overload aborts and reports to the tracker when the body throws") {
    WritebackHarness h({"pluginA"});
    h.tracker.open();
    plume::data::ModelDataView view = h.view("pluginA");
    EXPECT(!h.tracker.hasErrors());
    ...
    // The scope was destroyed without commit() → the abort was reported to the tracker.
    EXPECT(h.tracker.hasErrors());
...
CASE("test writeback scope - writeParam(name) error branches: no tracker, missing param, non-field") {
    // Do not attach the tracker yet: the first branch checks the pre-attach failure.
    WritebackHarness h({""}, WriteBackPolicy::single_writer, /*attach=*/false);
    h.data.createParam<int>("scalar", 7);

    // No tracker attached yet → BadValue.
...
CASE("test writeback scope - moved-from WriteScope is neutralised") {
...

    {
        ...
        // would report a spurious abort to the tracker.
    }

    EXPECT(!h.tracker.hasErrors());  // proves the moved-from src did not report an abort
...
CASE("test writeback scope - destroying a filtered view does not detach the model tracker") {
    WritebackHarness h({"", "pluginA"});  // model-facing "" consumer + plugin consumer
    h.tracker.open();

    {
        // A filtered view borrows the tracker; when it goes out of scope its base destructor must be a no-op w.r.t.
        // the shared tracker (ownsTracker_ == false).
        plume::data::ModelDataView view = h.view("pluginA");
        EXPECT(view.hasParameter("F"));
    }

    // If the view had wrongly detached, this model-facing write would throw "tracker not attached".
...
CASE("test writeback scope - WriteScope destroyed without commit() does not notify observers") {
...

    {
        ...
        // scope destroyed here without commit() — abort path, reports to the tracker.
    }
