...
CASE("test model data - observing atlas fields") {
    ...
    EXPECT_EQUAL(oberserverView(3), 4);
...
CASE("test model data - model-facing updateParam on Plume-owned atlas fields") {
    ...
    // Create a Plume-owned field seeded with initial values. The temporary is cloned into the parameter,
    // so the caller need not keep any handle alive afterwards.
    std::vector<int> initial{1, 2, 3, 4};
    data.createParam("owned", atlas::Field("owned", initial.data(), atlas::array::make_shape(initial.size())));
