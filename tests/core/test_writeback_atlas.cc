 ...
 *
 * The write-back lifecycle is driven directly through the tracker here (rather than the Manager/plugin machinery)
...
CASE("test writeback - provided atlas field is written back in place") {
    ...
    // Wire the write-back tracker directly, authorising the (empty-named) model-facing consumer to write "F".
    ...
    // Complete the write-back cycle cleanly before the tracker is destroyed.
    tracker.submit();
...
CASE("test writeback - plugin clones a FieldView, read-modify-writes, and lands in place") {
    ...
    // Wire the tracker authorising a named plugin consumer to write "F", then build the plugin-facing view.
    WriteAuthorisation auth;
 ...
 *        reported to the tracker (the slot goes to error) rather than silently corrupting the model buffer.
 */
CASE("test writeback - value writeParam rejects a shape/datatype-mismatched field and reports the error") {
    ...
    // Wrong shape: 2 elements vs the stored 4 → UserError, and the failure is recorded on the tracker.
